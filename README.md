...
### 연관관계 맵핑 분석
  ...
  - OrderProduct <> Order: 주문상품<>주문은 다대일 양방향 관계이다. 외래키가 주문상품에 있으므로 연관관계의 주인이다. 이에 따라 OrderItem.order를 order_item.order_id 외래키와 맵핑한다.
  - OrderProduct <> Product: 다대일 단방향 관계이다. OrderItem.item을 테이블 orders.item_id 외래키와 맵핑한다.
